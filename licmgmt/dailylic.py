...
for line in open('/home/pgrant/workarea/python/arm-20140715-095116.log'):
    ...
    if "TIMESTAMP" in tokenList:
       dateSet = 1
#       if len(maxCountTable) > 0 and max(maxCountTable.values()) > maxLic:
#           maxLic = max(maxCountTable.values())
       for licName in lastMaxCountTable.keys():
           if lastMaxCountTable[licName] < maxCountTable[licName]:
               print("\n\n\n")
               print("date:" + tokenList[-1], " @ ", tokenList[0])
               print("table: ")
               ...
               break
       lastMaxCountTable = deepcopy(maxCountTable)
    if dateSet == 1:
        if "OUT:" in tokenList:
